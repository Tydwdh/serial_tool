...
use super::{ContainerKind, ResizeState, SimplificationOptions, Tile, TileId, Tiles, UiResponse};
...
/// Exact destination selected by the drag-and-drop engine.
///
/// `target_rect` is the unsmoothed destination used for deciding the drop,
/// while `preview_rect` is its animated visual representation.
#[derive(Clone, Copy, Debug)]
pub struct DragPreview {
    pub target_tile_id: TileId,
    pub insertion_kind: ContainerKind,
...
pub trait Behavior<Pane> {
    ...
    fn paint_drag_preview(&self, visuals: &Visuals, painter: &egui::Painter, preview: DragPreview) {
        let preview_stroke = self.drag_preview_stroke(visuals);
        let preview_color = self.drag_preview_color(visuals);

        if let Some(parent_rect) = preview.parent_rect {
