...
use crate::behavior::{DragPreview, EditAction};
...
impl<Pane> Tree<Pane> {
    ...
    fn preview_dragged_tile(
        ...
        if let (Some(target_rect), Some(insertion_point)) =
            (drop_context.preview_rect, drop_context.best_insertion)
        {
            let preview_rect = smooth_preview_rect(ui, dragged_tile_id, target_rect);
            let parent_rect = self.tiles.rect(insertion_point.parent_id);
            let preview = DragPreview {
                target_tile_id: insertion_point.parent_id,
                insertion_kind: insertion_point.insertion.kind(),
                parent_rect,
                target_rect,
                preview_rect,
            };

            behavior.paint_drag_preview(ui.visuals(), ui.painter(), preview);
