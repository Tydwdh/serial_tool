...
- **实时终端**：文本/HEX 显示、时间戳、多行渲染、跨包拼接、搜索过滤、暂停接收（冻结画面查看）、CSV/JSONL 导出。
...
- **命令面板**：Ctrl+K 搜索并执行任意内置命令、插件命令或历史发送。
...
- Windows: `%APPDATA%\HardwareWorkbench\workspace.json`
- macOS: `~/Library/Application Support/HardwareWorkbench/workspace.json`
- Linux: `~/.config/HardwareWorkbench/workspace.json`
